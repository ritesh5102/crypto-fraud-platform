...
FEATURE_NAMES = [
    "amount", "frequency", "wallet_age_days", "num_receivers",
    "time_gap_hours", "hour_of_day", "is_round_amount",
]
...
class TransactionFeatures:
    ...
    def to_array(self) -> list[float]:
        return [
            self.amount, self.frequency, self.wallet_age_days,
            float(self.num_receivers), self.time_gap_hours,
            float(self.hour_of_day), float(self.is_round_amount),
        ]
...
def extract_features(tx: dict[str, Any], history: list[dict] | None = None) -> TransactionFeatures:
    ...
    return TransactionFeatures(
        amount=amount, frequency=frequency, wallet_age_days=wallet_age,
        num_receivers=num_receivers, time_gap_hours=time_gap,
        hour_of_day=ts.hour, is_round_amount=is_round,
    )
