...
def _init_ml():
    ...
    import numpy as np
    from services.fraud_model import _generate_synthetic_data  # noqa: SLF001
...
def _analyze_transaction(tx: dict) -> dict:
    ...
    graph_analyzer.add_transaction(
        tx["wallet_from"], tx["wallet_to"], float(tx["amount"]), tx["transaction_id"]
    )
...
async def health():
    return {
        ...
        "transactions_cached": len(_in_memory_tx),
        "timestamp": utc_now_iso(),
    }


@router.get("/live-price")
async def live_price(symbol: str = Query("BTC/USDT")):
    price = fetch_live_price(symbol)
    history = fetch_price_history(symbol, 24)
    return {"current": price, "history": history}
...
async def analyze(body: AnalyzeRequest):
    from utils.helpers import generate_tx_id
    ...
    tx = {
        ...
        "wallet_age_days": body.wallet_age_days,
        "num_receivers": body.num_receivers,
        "time_gap_hours": body.time_gap_hours,
        "timestamp": utc_now_iso(),
    }
...
async def analyze_batch(count: int = Query(5, ge=1, le=20)):
    results = []
    for tx in generate_batch(count):
        results.append(_analyze_transaction(tx))
...
async def fraud_alerts(limit: int = Query(30, ge=1, le=100)):
    try:
        ...
        pass
    alerts = _in_memory_alerts[:limit]
...
async def analytics():
    ...
    if not txs:
        return {"fraud_vs_safe": [], "risk_distribution": [], "avg_risk": 0}
    ...
    avg_risk = sum(t.get("risk_score", 0) for t in txs) / len(txs)
